...
class VampireSurvivorLite:
    def __init__(self, root: tk.Tk):
        ...
        # 배경 이미지 (같은 폴더의 background.png 사용)
    ...
    def update_monsters(self):
        ...
        if (not self.game_finished) and self.wave < self.max_wave and timer_now >= self.next_wave_time:
            # 새 웨이브 시작: 기존 일반/정예 몬스터 제거(보스만 남기고), 보스 스폰
    ...
    def draw_timer(self):
        # 상단 중앙에 경과 시간 표기 (일시정지/레벨업/게임오버 중에는 멈춘 시간 표시)
    ...
    def draw_bibles(self):
        for b in self.bibles:
            ...
            # 아주 작고 투명해지는 단계는 표시하지 않아 작은 점이 보이지 않도록 함
            if fade < 0.35:
                continue
    ...
    def draw_player(self):
        # 캐릭터 이미지를 중앙 앵커로 그림 (애니메이션 프레임 우선)
        frames = self._get_current_frames()
        if frames:
            if self.player_anim_index >= len(frames):
                self.player_anim_index = 0
    ...
    def game_loop(self):
        ...
        if not self.paused and not self.game_over:
            ...
            # 3) 단검, 몬스터, 바이블 업데이트 (레벨업 선택 중에는 정지)
            if not self.level_up_active:
